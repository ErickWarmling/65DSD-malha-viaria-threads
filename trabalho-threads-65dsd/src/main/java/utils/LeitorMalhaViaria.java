...
public class LeitorMalhaViaria {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(caminhoArquivo)))) {
            ...
            for (int i = 0; i < linhas; i++) {
                ...
                for (int j = 0; j < colunas; j++) {
                    ...
                    boolean isEntrada = identificarEntradas(direcao.getSentidoDirecao(), i, j, linhas, colunas);
    ...
    private boolean identificarEntradas(int direcao, int linha, int coluna, int linhas, int colunas) {
        boolean entradaBaixo = direcao == TipoCelula.ESTRADA_BAIXO && linha == 0;
        boolean entradaCima = direcao == TipoCelula.ESTRADA_CIMA && linha == linhas - 1;
        boolean entradaDireita = direcao == TipoCelula.ESTRADA_DIREITA && coluna == 0;
        boolean entradaEsquerda = direcao == TipoCelula.ESTRADA_ESQUERDA && coluna == colunas - 1;
